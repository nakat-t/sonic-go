...
func testProcessedAudioMatchesReference(t *testing.T, volume, speed, pitch float32, quality int, referenceFileName string) {
	...
	// For Debug: Output processed wave file to 'test/testdata/processed/'
	if os.Getenv("CGOSONIC_TEST_DEBUG") != "" {
		os.MkdirAll(filepath.Join(cwd, "../../test/testdata/processed/"), 0755)

		processedWavPath := filepath.Join(cwd, "../../test/testdata/processed/", referenceFileName)
